...
export const analyzeImage = async (base64Image: string, mimeType: string = 'image/jpeg'): Promise<string> => {
  try {
    ...
    throw new Error("Unable to analyze the image. Please ensure it is a clear medical image and try again.");
  }
};
...
export const analyzeMedicalDocument = async (base64Image: string, mimeType: string = 'image/jpeg'): Promise<string> => {
  try {
    ...
    if (error instanceof Error && error.message.includes("valid medical lab report")) throw error;
    throw new Error("Unable to read the document. Please ensure the text is clear and legible, then try again.");
  }
};
...
export const transcribeAudio = async (base64Audio: string): Promise<string> => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          { text: "Transcribe the following audio accurately into text. Provide only the transcription." }
        ]
      }
    });
    ...
    throw new Error("Unable to transcribe audio. Please speak clearly and check your microphone connection.");
  }
};
...
export const generateSpeech = async (text: string): Promise<string> => {
  try {
    ...
    throw new Error("Unable to generate speech at this time. Please check your internet connection.");
  }
};