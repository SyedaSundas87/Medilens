...
import { FileText, Upload, CheckCircle2, ArrowRight, Loader2, AlertCircle, RefreshCw, FileImage, Activity, ShieldCheck, Brain, ChevronRight, AlertTriangle } from 'lucide-react';
...
const LabReportAnalyzer: React.FC<LabReportAnalyzerProps> = ({ initialEmail = '' }) => {
  const [step, setStep] = useState<'upload' | 'review' | 'result'>('upload');
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  const [imageBase64, setImageBase64] = useState<string | null>(null);
  const [mimeType, setMimeType] = useState<string>('image/jpeg');
  const [extractedText, setExtractedText] = useState<string>('');
  const [patientEmail, setPatientEmail] = useState<string>(initialEmail);
  ...
  const handleAnalyze = async () => {
    ...
    if (!disclaimerAccepted) {
      setError("Please accept the medical disclaimer before continuing.");
      return;
    }
  ...
  const handleSubmitToSystem = async () => {
    ...
    try {
      // Step 2: Send to N8n
      const response = await sendLabReportData(extractedText, patientEmail);
      setResult(response);
      setStep('result');
    } catch (e: any) {
      console.error(e);
        ...
        {step === 'upload' && (
          <div className="p-8 md:p-12 flex flex-col items-center justify-center h-full animate-[fadeIn_0.3s_ease-out]">
            ...
            <div className="w-full max-w-xl mb-6 space-y-4">
              ...
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-teal focus:border-transparent outline-none transition-all"
                  required
                />
                <p className="text-[10px] text-gray-400 mt-2 italic">Results will be linked to this email address.</p>
              </div>

              {/* Legal Disclaimer Notice */}
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-900 text-xs space-y-2 shadow-sm">
                <div className="flex items-center gap-2 font-bold text-amber-800">
                  <AlertTriangle size={16} className="text-amber-500" />
                  Medical Report Interpretation Disclaimer
                </div>
                <div className="text-amber-800/80 leading-snug">
                  <p>Explanations provided are for informational guidance only. <strong>This tool does not provide medical diagnosis or clinical decisions.</strong> Consult your doctor before making medical decisions.</p>
                </div>
                <label className="flex items-center gap-2 pt-1 cursor-pointer group">
                  ...
                  <span className="font-medium group-hover:text-amber-900 transition-colors">
                    I agree this is for informational guidance only and not a diagnosis.
                  </span>
                </label>
              </div>
            </div>
            ...
            <button
              ...
              disabled={!imagePreview || loading || !disclaimerAccepted || !patientEmail}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 flex-grow min-h-[400px]">
              ...
              <div className="p-8 flex flex-col h-full overflow-hidden">
                <h3 className="font-bold text-gray-500 uppercase tracking-widest text-xs mb-4 flex items-center gap-2">
                    <FileText size={14} /> Digitized Content (Editable)
                </h3>
                <div className="bg-white rounded-xl border border-gray-200 flex-grow overflow-hidden shadow-inner flex flex-col">
                  <textarea 
                    className="w-full h-full p-6 font-mono text-sm text-gray-600 leading-relaxed outline-none resize-none"
                    value={extractedText}
                    onChange={(e) => setExtractedText(e.target.value)}
                    placeholder="Extracted text will appear here. You can edit it or add notes..."
                  />
                </div>
              </div>
            </div>
