...
Command Center UI	Complaints UI
Lab Report & Prescription Analyzer	RAG Knowledge Base Chat
Lab Analyzer UI	RAG Chat UI
Symptom Checker & Triage	Voice Appointment Agent
Symptom Checker UI	Voice Agent UI
Workflow Screenshots
Command Center	Complaint Management
Command Center Workflow	Complaint Management Workflow
Lab Report Analyzer	Voice Appointment Booking
Lab Analyzer Workflow	Voice Booking Workflow
Hospital Triage	RAG Knowledge Base
Hospital Triage Workflow	RAG Workflow
Retell Voice Agent	
Retell Voice Agent Workflow	
Problem Statement
...
Input arrives via voice call, HTTP webhook, or chat widget
n8n validates, authenticates, and routes the request
AI agents (Gemini) analyze intent and generate structured responses
Supabase/Postgres stores and retrieves all records
Email/Slack notifications are dispatched for confirmations and alerts
Responses are returned to patients, staff, or admin dashboards in real time
Workflow Modules
1. Hospital Admin Command Center
...
Validates API key → returns 401 if invalid
Checks current suggestion status → returns early if already processed
Routes to Approve or Reject branch via a Switch node
Updates ai_suggestions table with the new status
Determines the correct primary key for the related table (patients, doctors, beds, etc.)
Updates the ai_suggestion field on the corresponding entity record
...
Voice Appointment Booking Workflow
...
Symptom Checker & Triage Workflow
...
AI Triage Flow:
Specialist Selector (Gemini + session memory) — Analyzes symptoms and outputs exactly one specialist name from a list of 11 (Dermatologist, Cardiologist, Neurologist, ENT, Gastrologist, Endocrinologist, Gynecologist, Orthopedic, General Physician, Pediatrician, Psychologist)
Specialist Prompt Builder — Dynamically constructs a system prompt specific to the selected specialist, including their emergency trigger conditions
Dynamic Specialist Agent (Gemini) — Performs full clinical assessment and returns structured JSON with: diagnosis, severity, homeRemedies, herbalTreatments, yoga, exercise, lifestyleChanges, diet, firstAid, emergencyReason
Parse & Normalize — Strips markdown fences, extracts JSON, normalizes all array fields, and provides a safe fallback if parsing fails
Emergency Protocol:
...
RAG Knowledge Base Agent Workflow
...
Searches a specific Google Drive folder for all PDF files
Processes PDFs in batches to avoid memory limits
Downloads each PDF as binary data
Loads and splits document text into overlapping chunks (200-character overlap for context preservation)
Generates embeddings using Gemini Embedding 001 (models/gemini-embedding-001)
Upserts all vectors into a Pinecone index under the medilens namespace
...
Validates API key
Routes to the RAG Agent (Gemini + Pinecone tool + PostgreSQL chat memory)
The agent uses the Pinecone Vector Store as a retrieval tool (top-5 results, medilens namespace)
If the knowledge base does not contain the answer, the agent responds with: "I'm sorry, I could not find this information in our hospital knowledge base" — no guessing or hallucination
All interactions are logged to rag_logs in Supabase
Responds to the website via webhook
...
Retell AI Voice Agent Conversation Flow
...
Greets the patient and asks for their full name (with spelling confirmation)
Collects phone number (reads back in grouped digits)
Collects email address (spells username letter by letter + domain)
Asks which department they want to book with
Says: "I will check the available slots" → triggers tool call
Tool Calls:
...
Asks for patient name (with spelling confirmation)
Asks for appointment ID
Triggers appointment lookup → reads appointment details back to patient
Asks: "Do you want me to proceed with cancellation?"
If YES → triggers cancellation; if NO → politely ends
Tool Calls:
...
Asks for patient name (with spelling confirmation)
Asks for appointment ID
Looks up current appointment → reads details back
Asks: "Do you want me to proceed with rescheduling?"
If YES → checks next available slot and presents it to patient
Patient confirms → reschedule is confirmed
Tool Calls:
...
Asks for patient name (with spelling confirmation)
Asks for complaint ID
Triggers complaint lookup → reads status back to patient
Tool Call:
...
Upload Medilens_Voice_Agent_Retell.json
After import, go to each component's Tools section and update all webhook URLs from YOUR_N8N_INSTANCE_URL to your actual n8n production webhook base URL
If using a Retell knowledge base for the FAQs component, create a knowledge base in Retell, copy its ID, and replace YOUR_KNOWLEDGE_BASE_ID in the FAQs node instruction
Set your Agent ID (auto-generated on import — note it down for API calls)
Test using Retell's built-in call simulator before going live
Tech Stack
Layer	Technology
...
In n8n, go to Workflows → Import from File for each file
...
Upload all hospital policy/service PDF documents to the configured Google Drive folder
Open the RAG ingestion production part 1 workflow in n8n
Click Execute Workflow to run the ingestion manually
Verify that vectors appear in your Pinecone index under the medilens namespace
...
Log in to your Retell AI Dashboard
Navigate to Agents → Import Agent and upload Medilens_Voice_Agent_Retell.json
Once imported, open the agent and go to each component's Tools section
Update every webhook URL — replace YOUR_N8N_INSTANCE_URL with your actual n8n production base URL across all four components:
...
If using a Retell knowledge base for FAQs, create one in Retell, copy the generated ID, and replace YOUR_KNOWLEDGE_BASE_ID in the FAQs component conversation node instruction
Ensure all tools send the x-retell-signature header (Retell handles this automatically)
Use Retell's built-in Call Simulator to test each flow before going live
API Reference
Command Center
...
Fork the repository
Create a new branch (feature/your-feature or fix/issue-description)
Make your changes with clear, descriptive commit messages
Test all affected workflows end-to-end before submitting
