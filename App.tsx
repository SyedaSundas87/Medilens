...
import { Activity, Heart, Menu, X, Volume2, Pause, Loader2, LogIn, UserPlus, Leaf, Utensils, Sun, Zap, Wind, PlusCircle, AlertTriangle, ArrowRight, Stethoscope, Home, CheckCircle2, Info, FileText, Building2, FlaskConical, MessageSquare, Clock, Plus, Phone, ArrowUpRight, Lock, AlertCircle } from 'lucide-react';
...
const App: React.FC = () => {
  const [view, setView] = useState<'home' | 'admin' | 'patient' | 'lab-analysis' | 'hospital-info' | 'medical-disclaimer'>('home');
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (path === '/medical-disclaimer') {
        setView('medical-disclaimer');
      } else if (path === '/lab-analysis') {
        setView('lab-analysis');
      } else if (path === '/hospital-info') {
        setView('hospital-info');
      } else {
        setView('home');
      }
    };
  ...
  const navigateTo = (newView: any, path: string) => {
  ...
  if (view === 'admin') {
    return (
      <AdminDashboard 
        ...
        onViewChange={(v) => {
          if (v === 'medical-disclaimer') navigateTo('medical-disclaimer', '/medical-disclaimer');
          else if (v === 'lab-analysis') navigateTo('lab-analysis', '/lab-analysis');
          else if (v === 'hospital-info') navigateTo('hospital-info', '/hospital-info');
  ...
  return (
    <div className="font-sans text-gray-800 bg-cream min-h-screen flex flex-col">
      ...
      />
      <ComplaintWidget />
      <RagChatWidget user={currentUser} />
      <header className="sticky top-0 z-40 bg-cream/90 backdrop-blur-md">
        <div className="container mx-auto px-4 py-6 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex items-center gap-10">
            <button onClick={() => { navigateTo('home', '/'); scrollToSection('home'); }} className={`text-sm font-medium transition-colors ${view === 'home' ? 'text-teal' : 'text-text-body hover:text-teal'}`}>Home</button>
            <button onClick={() => { navigateTo('home', '/'); scrollToSection('symptoms'); }} className="text-sm text-text-body hover:text-teal font-medium transition-colors">Symptoms</button>
            <button onClick={() => navigateTo('lab-analysis', '/lab-analysis')} className={`text-sm font-medium transition-colors ${view === 'lab-analysis' ? 'text-teal' : 'text-text-body hover:text-teal'}`}>Lab Reports</button>
            <button onClick={() => navigateTo('hospital-info', '/hospital-info')} className={`text-sm font-medium transition-colors ${view === 'hospital-info' ? 'text-teal' : 'text-text-body hover:text-teal'}`}>Contact</button>
            <button onClick={() => navigateTo('medical-disclaimer', '/medical-disclaimer')} className={`text-sm font-medium transition-colors ${view === 'medical-disclaimer' ? 'text-teal' : 'text-text-body hover:text-teal'}`}>Disclaimer</button>
            {currentUser ? (
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-t border-gray-100 p-4 flex flex-col gap-4 shadow-lg absolute w-full z-50">
            <button onClick={() => { navigateTo('home', '/'); scrollToSection('home'); }} className="text-left hover:text-teal py-2">Home</button>
            <button onClick={() => { navigateTo('home', '/'); scrollToSection('symptoms'); }} className="text-left hover:text-teal py-2">Symptoms</button>
            <button onClick={() => { navigateTo('lab-analysis', '/lab-analysis'); setMobileMenuOpen(false); }} className="text-left hover:text-teal py-2">Lab Reports</button>
            <button onClick={() => { navigateTo('hospital-info', '/hospital-info'); setMobileMenuOpen(false); }} className="text-left hover:text-teal py-2">Contact</button>
            <button onClick={() => { navigateTo('medical-disclaimer', '/medical-disclaimer'); setMobileMenuOpen(false); }} className="text-left hover:text-teal py-2">Medical Disclaimer</button>
      ...
      {view === 'patient' && currentUser ? (
        <PatientDashboard 
          ...
          appointments={appointments} 
          onLogout={handleLogout} 
          onBookNew={() => { navigateTo('home', '/'); setTimeout(() => scrollToSection('doctors'), 100); }} 
          onViewChange={(v) => {
            if (v === 'medical-disclaimer') navigateTo('medical-disclaimer', '/medical-disclaimer');
            else if (v === 'lab-analysis') navigateTo('lab-analysis', '/lab-analysis');
            else if (v === 'hospital-info') navigateTo('hospital-info', '/hospital-info');
        ...
        <main className="flex-grow bg-white"><div className="container mx-auto px-4 py-8"><div className="mb-6"><button onClick={() => navigateTo('home', '/')} className="flex items-center gap-2 text-text-body hover:text-teal transition-colors"><ArrowRight className="rotate-180" size={18} /> Back to Home</button></div><LabReportAnalyzer initialEmail={currentUser?.email} /></div></main>
      ) : view === 'hospital-info' ? (
        <main className="flex-grow bg-white"><div className="container mx-auto px-4 py-8"><div className="mb-6"><button onClick={() => navigateTo('home', '/')} className="flex items-center gap-2 text-text-body hover:text-teal transition-colors"><ArrowRight className="rotate-180" size={18} /> Back to Home</button></div><HospitalInfo /></div></main>
      ) : view === 'medical-disclaimer' ? (
        <main className="flex-grow bg-white"><MedicalDisclaimer onBack={() => navigateTo('home', '/')} /></main>
      ) : (
        <>
            ...
            <div className="container mx-auto px-4 relative z-10 flex flex-col items-center text-center">
                <div className="max-w-3xl animate-[fadeIn_0.5s_ease-out]">
                    ...
                    <div className="flex flex-wrap gap-4 justify-center">
                        ...
                        <button onClick={() => navigateTo('hospital-info', '/hospital-info')} className="border border-teal text-teal hover:bg-teal hover:text-white px-8 py-4 text-sm font-bold uppercase tracking-wider transition-all">Learn More</button>
                    </div>
                </div>
            </div>
          ...
          <section id="symptoms" className="py-24 container mx-auto px-4">
             <div className="text-center mb-16"><span className="text-teal font-serif italic text-lg block mb-2">Our Technology</span><h2 className="text-4xl font-serif text-text-main mb-4">Smart Symptom Checker</h2><div className="w-20 h-0.5 bg-teal mx-auto"></div><p className="text-text-body mt-6 max-w-2xl mx-auto">Use our advanced AI to analyze your symptoms via text, voice, or images. Get instant wellness guidance.</p></div>
              <SymptomChecker onResult={handleSymptomResult} initialEmail={currentUser?.email} />
              {aiResponse && (
                  ...
                  <div className={`bg-white shadow-2xl p-8 md:p-12 relative overflow-hidden ${aiResponse.triage_level === 'emergency' ? 'border-l-8 border-red-500' : aiResponse.triage_level === 'urgent' ? 'border-l-8 border-orange-400' : 'border-l-8 border-teal'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-8 border-t border-gray-100">
                      <div className="bg-gray-50 p-6">
                        <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Recommendation</span>
                        <div className="flex justify-between items-center">
                          <span className="text-xl font-serif text-teal">{aiResponse.specialty_recommendation}</span>
                          <button onClick={() => handleSpecialistClick(aiResponse.specialty_recommendation)} className="text-sm font-bold text-text-main hover:text-teal underline">Book Now</button>
                        </div>
                      </div>
                      <div className={`p-6 ${aiResponse.triage_level === 'emergency' ? 'bg-red-50' : aiResponse.triage_level === 'urgent' ? 'bg-orange-50' : 'bg-green-50'}`}>
                        <span className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Triage Status</span>
                        <span className={`text-xl font-bold uppercase ${aiResponse.triage_level === 'emergency' ? 'text-red-600' : aiResponse.triage_level === 'urgent' ? 'text-orange-600' : 'text-green-600'}`}>{aiResponse.triage_level}</span>
                      </div>
                    </div>

                    {/* Legal Warning */}
                    <div className="mt-8 pt-6 border-t border-gray-100">
                      <div className="flex gap-3 p-4 bg-amber-50 border border-amber-100 rounded-xl text-amber-800 text-sm leading-relaxed">
                        <AlertCircle className="shrink-0 text-amber-500" size={20} />
                        <p>
                          <strong>⚠️ Legal Disclaimer:</strong> This output is AI-generated and does not constitute medical advice, diagnosis, or treatment. Always consult a licensed healthcare professional before making medical decisions.
                        </p>
                      </div>
                    </div>
                  </div>
